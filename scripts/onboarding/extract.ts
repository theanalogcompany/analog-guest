...
- venue_info vs voice_corpus vs knowledge_corpus — three places content can land, with three different jobs. Read this carefully before placing anything:
    ...
    - voice_corpus = HOW the venue talks to guests — captured from what the operator actually says or writes TO a guest, not descriptions of their style, and not limited to texts. Two kinds of content qualify: (1) a real or near-verbatim TEXT the operator actually sent or would send to a guest, or (2) a short SPOKEN line addressed to a guest — said in second person, or something the operator would plausibly say to a guest across the counter: a recommendation, an invitation, a house rule said warmly, a one-line explanation. One or two sentences, trimmed of filler. Texting-specific conventions (length, emojis, tone) already live in brand_persona — voice_corpus is about what gets said, not the channel it's said through. Do NOT include: descriptions of style ("I always use emojis", "I keep it short") — those go in brand_persona (tone/lengthGuide/emojiPolicy), never here as if they were examples. Do NOT include joking, sarcastic, or hypothetical scenario answers ("I'm at their house", "if a guest asked X I guess I'd say..."). Do NOT include long narrative or reflective passages about the business's history, mission, or sourcing, or the operator explaining a perk or policy TO THE INTERVIEWER rather than saying it TO a guest — that belongs in knowledge_corpus. If the interview doesn't include qualifying content, voice_corpus should be sparse or even empty — that's correct, not a failure. Never synthesize a message or a line from scratch to fill a quota.
...
- For voice_corpus: extract every entry that is either (a) a real or near-verbatim TEXT the operator sent or would send to a guest, or (b) a short SPOKEN line addressed to a guest — second person, or something the operator would plausibly say to a guest across the counter (a recommendation, an invitation, a house rule, a one-line explanation), trimmed to one or two sentences with filler removed. source_type='voicenote_transcript' for either kind. Do NOT use source_type='manual_entry' for voice_corpus under any circumstance — a synthesized line the operator never said or reviewed is not evidence of their voice, however plausible it sounds. There is no target count and no floor to hit here: extract every qualifying entry the transcript supports, however many that is, including zero. Do not manufacture entries to reach 5 — a shortfall is reported in Needs confirmation, not papered over here. confidence_score: 0.95 for direct verbatim quotes (text or spoken), 0.9 for tight paraphrases. voice_corpus tags = situation/style ('welcome', 'follow_up', 'perk_surface', 'anti_pattern'); single \`tags\` array.
