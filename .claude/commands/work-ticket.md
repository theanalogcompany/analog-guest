...
14. **Continue the ticket's branch if it exists** (`branchExists`, locally or on GitHub), and never check it out in this checkout: CI refuses checking out a branch by name, because no allowlist rule can admit a branch name without also admitting a path after it, which discards work (TAC-471). Below, `<branch>` is the name `branchExists` listed with any `remotes/origin/` prefix removed: `remotes/origin/jaipal/tac-325-order-capture` is `jaipal/tac-325-order-capture`.
    - **If this checkout is already on it** (`git branch --show-current` prints it), carry on here.
    ...
    - **To change its files, use a worktree**: `git worktree add .worktrees/<branch> <branch>`. For a branch that is only on GitHub, this creates the local branch tracking `origin/<branch>`. Then `cd .worktrees/<branch>`, in a command of its own, and stay there for the rest of the session: commits, `git push` (to the ticket's branch) and `gh pr create` all run from there, and nothing needs installing, because `npx` finds this checkout's `node_modules`.
    - **The pre-commit hook does not run in a worktree**, because husky's hook directory is created by `npm install` and a new worktree never has one. Before each commit there, run `npx tsc --noEmit`, `npm run lint` and `npx vitest run` yourself.
    - **Remove the worktree before running anything in this checkout again**: while it is there, `npx vitest run` and `npm run lint` in this checkout collect its copy too. A local session goes back with `cd` and the checkout's absolute path, then runs `git worktree remove .worktrees/<branch>`.
