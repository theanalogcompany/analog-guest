...
14. **Continue the ticket's branch if it exists** (`branchExists`, locally or on GitHub), without checking it out in this checkout (ruled 2026-09-18, TAC-471). Below, `<branch>` is the name `branchExists` listed with any `remotes/origin/` prefix removed: `remotes/origin/jaipal/tac-325-order-capture` is `jaipal/tac-325-order-capture`.
    ...
    - **To change its files, use a worktree**: `git worktree add .worktrees/<branch> <branch>`. For a branch that is only on GitHub, this creates the local branch tracking `origin/<branch>`. If the local branch already existed, bring it up to date there with `git pull --ff-only`. If git says the branch is already checked out somewhere else, that is another local session's worktree or a leftover: leave it and ask. Then `cd .worktrees/<branch>`, in a command of its own. The shell stays there, and from then on:
      - **Every path you give a tool is under `.worktrees/<branch>/`.** Read, Edit, Write, Grep and Glob take their own paths and do not follow the shell, so a path in this checkout reads or edits `main`'s copy, which nothing commits.
      - **A subagent starts in this checkout**, not the worktree. Give it the worktree's absolute path, and tell it to run git from there in one command (`cd` to that path, `&&`, then the git command).
