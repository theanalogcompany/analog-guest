...
- **`[NEEDS-INPUT]` AND `branchExists === true`** → Phase 3 question answered. Check the branch out (step 14), read `git log` and `git diff` against main to establish what is done and what is left, then continue.
...
14. **Continue the ticket's branch if it exists** (`branchExists`, locally or on GitHub). Check it out by the name `branchExists` listed, with any `remotes/origin/` prefix removed: `remotes/origin/jaipal/tac-325-order-capture` becomes `git checkout jaipal/tac-325-order-capture`, which checks out a GitHub-only branch as a local one. Then `git log --oneline origin/main..HEAD` and `git status` show what an earlier session already did. Otherwise create it: `jaipal/tac-xxx-short-description`, with the ticket id in lowercase.
