...
14. **Continue the ticket's branch if it exists** (`branchExists`, locally or on GitHub), without checking it out in this checkout (ruled 2026-09-18, TAC-471). Below, `<branch>` is the name `branchExists` listed with any `remotes/origin/` prefix removed: `remotes/origin/jaipal/tac-325-order-capture` is `jaipal/tac-325-order-capture`.
    ...
    - **To change its files, use a worktree**: `git worktree add .worktrees/<branch> <branch>`. For a branch that is only on GitHub, this creates the local branch tracking `origin/<branch>`. If git says the branch is already checked out somewhere else, that is another local session's worktree or a leftover: leave it and ask. Then `cd .worktrees/<branch>`, in a command of its own. The shell stays there. If the local branch existed before this step, bring it up to date now with `git pull --ff-only`, and if that fails, stop and say so. From then on:
      - **Every path you give a tool is under `.worktrees/<branch>/`.** Read, Edit and Write take absolute paths, so a path in this checkout reads or edits `main`'s copy, which nothing commits. Give Grep and Glob the worktree's path too, rather than rely on where they start.
      - **A subagent starts in this checkout**, not the worktree. Give it the worktree's absolute path, and tell it to start every command it runs, git, tests and lint alike, with `cd` to that path and `&&`.
      - **Write to Linear from this checkout.** From the worktree, `node scripts/linear.mjs` runs the branch's copy of the helper, which may predate it or be what the ticket changes. `cd` to this checkout's absolute path, write, then `cd` back into the worktree, each in a command of its own.
      - **The pre-commit hook does not run.** husky makes its hook directory when dependencies are installed, which never happens in a new worktree. Before each commit, run `npx tsc --noEmit`, `npm run lint` and `npx vitest run` yourself. They find this checkout's `node_modules`, so nothing needs installing, unless the branch changes dependencies: `npm install` is refused, so say so and stop rather than work around it.
      - Commits, `git push` (to the ticket's branch) and `gh pr create` run from the worktree.
      - **A turn-limit notice cannot see the worktree.** Its list of changes never committed reads this checkout only, so work left uncommitted there is lost without being listed. Commit and push each planned commit as you make it (step 15).
    - **Remove the worktree before running tests or lint in this checkout**: while it is there, `npx vitest run` and `npm run lint` here also collect its copy. From this checkout, `git worktree remove .worktrees/<branch>`; it refuses while the worktree holds untracked files.
