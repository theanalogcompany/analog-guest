...
- **Every `mid`** was rebuilt with made-up IDs, at the same length and format, and it still decodes the same way (see below). The echo and the read share one `mid`, as they did in the capture. What is kept from the real `mid`s: the fixed 34-character header, the `ZDZD` padding, and the leading digits of the thread ID (`3402823668`) and message ID (`33014212`). Those keep the IDs in the ranges Meta uses; thread IDs sit just below 2^128, and a message ID's high digits track send time, which the timestamps already give.
