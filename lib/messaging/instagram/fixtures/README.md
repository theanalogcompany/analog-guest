...
- **Not replaced in the postback:** the referral's `source` and `type`, which are the finding, and its `ref`, which arrived as `TESTVENUE` because the link carried that value. Its `title` and `payload` aren't replaced either. That was a judgment call beyond the ruling, which named neither: they are the venue's icebreaker configuration, not anything the guest wrote, even though the thread shows the title as the guest's sent message. They are safe here only because they are generic. A venue's icebreakers are visible to anyone who opens a DM with it, so a distinctive one would identify the venue even with every ID replaced; replace it in a future capture.
...
- **In this capture, a deleted and reopened thread got a referral but kept its thread ID.** Per Jaipal, the thread was deleted and then reopened through the link before this capture; which side deleted it isn't recorded, and the payload itself can't show the deletion. The referral fired as it does on a new thread, yet the thread ID inside its `mid` is the one the earlier messages carry. (Instagram's delete removes a chat only for whoever deleted it, so a surviving thread ID is expected if the guest's side deleted it.) On this path, an icebreaker tap into a chat that was empty again, a handler can't use a new thread ID to spot the guest coming back; the referral is the signal. Other ways back in weren't captured: following the link into a chat that still has messages shows no icebreakers, and a guest who types instead of tapping may or may not bring the referral.
