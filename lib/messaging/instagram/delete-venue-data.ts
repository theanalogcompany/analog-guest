...
//   review_state, category, channel, direction and ids, so
//   count_outbound_responses and the recognition signals that key on row
//   existence keep working against an anonymised shell.
//
...
// incidentally carry guest-volunteered free text. A full sweep of every jsonb
// free-text field in the schema is out of scope for this pass. The ruling's
// enumerated list plus message bodies is covered completely.
//
// IDEMPOTENT: a second request finds no guest with a real scoped id (they are
// all tombstoned), redacts nothing, records a second row and still responds
// in shape. Zero affected is a legitimate answer, not a failure.
