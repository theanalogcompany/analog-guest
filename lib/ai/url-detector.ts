 ...
 *
 * "/" is NOT in this set: it is handled separately by canonicalizeUrl, which
 * treats exactly one trailing slash as insignificant on BOTH sides of the
 * comparison.
 */
...
/**
 * Remove exactly one trailing "/" — the only normalization applied at match
 * time (ruled 2026-09-21).
 *
 ...
 * write. Nothing else is reconciled: scheme, case, query string and path stay
 * significant, so `http://lemils.com/` never matches a listed
 * `https://lemils.com/`.
 *
...
function isUrl(token: string): boolean {
  ...
  return canonicalizeUrl(token).includes('/')
}
 ...
 *
 * Exact string comparison, after trailing-noise trimming and after the single
 * trailing slash is reconciled on both sides. A one-character difference in a
 * slug is a different link and is reported.
 *
