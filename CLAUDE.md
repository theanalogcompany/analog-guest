...
- `lib/schemas/` — Zod schemas for venue-shaped JSONB fields. `BrandPersonaSchema` (handles dual-shape `voiceAntiPatterns`, see gotchas), `MessageReviewSchema` (the `response_review` shape from migration 014), `VenueInfoSchema` (with `filterActiveContext` per THE-150), `GuestContextSchema` + `GuestContextPatchSchema` (the persisted shape and the agent-emitted patch shape for `guests.context` per TAC-296, plus `filterActiveLifeContext` mirroring the venue-info expires_at filter, and `toParsedGuestContext` for the runtime-ready projection with expired entries dropped + observations truncated to `OBSERVATION_RENDER_LIMIT`=10). Read JSONB fields through these — never directly via SQL paths.
- `lib/guests/` — Per-guest helpers consumed by the agent runtime (TAC-296). `context.ts` exposes `getGuestContext(guestId)` (parsed JSONB read, fail-OPEN on malformed payload → empty context), `updateGuestContext({guestId, update, now})` (deep-merge patch + observation append + identity-column sync in a single multi-SET UPDATE), `deepMergeContext(existing, patch, now)` (pure), and `isEmptyContextUpdate(update)` (short-circuit predicate for the no-op emission shape). All `RAGResult`-shaped — never throws into the agent loop. Last-write-wins concurrency for v1, see "Common gotchas." The orchestrator dispatches `updateGuestContext` from `handle-inbound.ts` + `handle-followup.ts` **between `generateStage` success and `applyApprovalPolicyStage`** so context capture reflects what the agent UNDERSTOOD from the inbound regardless of whether the draft ships, queues, or refuses.
...
- `lib/notifications/` — APNs push pipeline for the operator approval queue (TAC-207). `apns/jwt.ts` signs the ES256 bearer token using `jose` (5 env vars: `APNS_AUTH_KEY` PEM, `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_BUNDLE_ID`, `APNS_ENV`); cached at module scope with a 50-minute refresh window and an explicit invalidation on key-id / team-id rotation. `apns/client.ts` wraps `node:http2` (new pattern — no prior http2 use in the repo) with one-shot `sendApnsRequest(deviceToken, payload)` returning a `RAGResult`-shaped result; one session per request, no pooling at pilot scale. `send.ts` is the orchestrator: `shouldSendDraftFlaggedPush(primaryTrigger)` filters to `model_flagged` / `comp_regex_backstop` / `fidelity_below_auto_send_floor` (skips `previous_pending_held` — regen of an already-pushed draft); `sendDraftFlaggedPush({agentRunId, venueId, guestId, guestFirstName, draftId, primaryTrigger})` fans out to every operator whose allowlist covers the venue, computes the badge using the same predicate as `list_operator_queue` (single source of truth — `review_state='pending' AND venue_id IN (operator's allowed venues)`), POSTs to APNs, handles 410 Gone / 400 BadDeviceToken by nulling the token columns. Never throws. Called fire-and-forget from `handle-inbound.ts` + `handle-followup.ts` via `waitUntil`. Payload contract with TAC-288 (operator client): `{ aps: { alert: {title, body}, badge, sound: "default" }, draftId, guestId, operatorId }` — no message contents, ever (privacy invariant, asserted in tests). Notification body is `"Reply to {firstName} — {context}"` where `context` is a categorical label (`needs review` / `comp request` / `low fidelity`), name-truncated to keep the assembled string ≤40 chars.
...
**Four triggers compose; any one queues.** Enumeration order (the order the gate evaluates and the order they appear in `triggers[]`):
