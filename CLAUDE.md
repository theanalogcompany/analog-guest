...
- `lib/voice-training/` — Shared corpus-write + anti-pattern dedupe-append helpers used by both the 08-flow onboarding script (`scripts/ingest-response-review.ts`) and the cc-review live-edit API route. `upsertCorpusEdit({...}, 'skip-existing' | 'replace')` handles voice_corpus upsert + Voyage embed with embed-failure rollback (no orphaned un-embedded rows). `dedupeAndAppendAntiPatterns(venueId, rules)` does read-modify-write on `venue_configs.brand_persona.voiceAntiPatterns` with case + whitespace normalization. Anti-corpus-poisoning rule lives here: only the operator-edited message text is embedded — never the rejected generation. THE-235.
