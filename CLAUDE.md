...
## Operator API (TAC-258 + TAC-207)

The mobile-operator approval queue is served by six route handlers on the **guest host** (the middleware host-gate only restricts `/admin/*` — operator-API paths flow through). When a dedicated operator host comes online, the routes can move under a host-rewrite without touching the handlers.

**Path convention.** Five TAC-258 queue routes live under `app/api/operator/*` (SINGULAR): `queue`, `messages/[id]/{approve,edit,skip,undo}`. The TAC-207 `POST /api/operators/devices` (APNs token registration) is the **one plural carve-out** because the analog-operator iOS client (TAC-288) shipped to TestFlight against the ticket's draft text (which said plural) before the server-side path was finalized. Once TestFlight is in operators' hands the URL is locked-in until force-update — moving the server is cheaper than version-gating the client. **Future operator-API endpoints follow the SINGULAR convention.** The plural is a one-off documented at the top of `app/api/operators/devices/route.ts`. Surfacing case: TAC-207 initially shipped at the singular path; UAT of TAC-288 surfaced the 404 with Vercel's fallback HTML response, server-side path was moved to plural in a hotfix PR.
