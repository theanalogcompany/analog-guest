...
The mobile-operator approval queue is served by six route handlers under `app/api/operator/*`, on the **guest host** (the middleware host-gate only restricts `/admin/*` — `/api/operator/*` flows through). When a dedicated operator host comes online, the routes can move under a host-rewrite without touching the handlers. Five TAC-258 queue routes (`queue`, `messages/[id]/{approve,edit,skip,undo}`) plus the TAC-207 `POST /api/operator/devices` (APNs token registration — singular path, NOT `/operators/devices` despite the ticket's draft text; matches the convention).
