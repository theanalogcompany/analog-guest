...
4. `previous_pending_held` — `hasPendingDraft` lookup against the migration 018 partial index finds an existing `review_state='pending'` row for the (venue, guest) pair. **Fails OPEN** on DB error: lookup throws or returns an error → returns `false` → gate proceeds to send. The single-pending invariant lives upstream in TAC-264; this is best-effort read-side enforcement.
...
**PostHog event:** `captureDraftQueued` (`lib/analytics/posthog.ts`) fires whenever the gate queues, with `triggers` + `primaryTrigger` + `voiceFidelity` + `modelRequiresApproval` + `modelApprovalReason` + `compRegexMatchedPattern` + `hasPreviousPending` + `kind` ('inbound' | 'followup') + `category` + bodies. Slack relay on (pilot ops needs visibility). Mirrors the `voice_fidelity_low` / `dash_violation_persisted` shape.
